...
// name `classroom_session` — underscore, not hyphen; PartyKit's codegen
// `import <name>Party` breaks on hyphens and rejects uppercase). This is the
// transport half of Classroom v2:
//
...
type NavMsg = {
  ...
  focusOpen: boolean;
  /** Deck slide index (interactive slides); absent = phase-level nav. */
  slideIndex?: number;
};
...
// Phase 3 / 4 state fan-out (API POSTs these after persisting the PATCH). The
// party is a pure relay: it forwards the payload and only decides the audience.
type PairsMsg = { type: 'pairs'; pairs: unknown };
type ShowcaseMsg = { type: 'showcase'; showcase: unknown };
type TimerMsg = { type: 'timer'; timer: unknown };
type MediaMsg = {
  type: 'media';
  slideId: string;
  action: 'play' | 'pause';
...
export default class ClassroomSessionServer implements Party.Server {
  ...
  private async route(msg: InboundMsg): Promise<void> {
    switch (msg.type) {
      ...
      case 'mode':
        ...
        break;
      case 'pairs':
        // Rule 2: pairs carry enrollment ids — never to the projector.
        this.broadcast(msg, ['teacher', 'student']);
        break;
      case 'showcase':
      case 'timer':
      case 'media':
        // Deliberate all-role broadcasts (featured project, countdown, video
        // control) — no identified data.
        this.broadcast(msg, ['teacher', 'student', 'projector']);
        break;
      case 'end':
        this.handleEnd();
  ...
  onMessage(message: string, sender: Party.Connection) {
    ...
    // The socket accepts no SESSION-STATE writes from clients. Exceptions are
    // ephemeral, non-state signals: heartbeats, and (Phase 4) a student's local
    // slide position in a student-paced deck — teacher-only, presence-like.
    try {
      ...
      if (data.type === 'ping') {
        ...
        // Rule 2: per-student position is identified — teacher only.
        this.broadcast(
          {
            type: 'position',
            at: this.nowIso(),
