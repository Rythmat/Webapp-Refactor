...
const PEDAL_CATALOG: PedalDef[] = [
  ...
  },
  {
    ...
    color: '#60a5fa',
    params: [
      { key: 'inputLevel', label: 'Input', default: 0.5 },
      { key: 'bass', label: 'Bass', default: 0.5 },
      { key: 'mid', label: 'Mid', default: 0.5 },
      { key: 'treble', label: 'Treble', default: 0.5 },
      { key: 'presence', label: 'Presence', default: 0.5 },
      { key: 'outputLevel', label: 'Output', default: 0.5 },
      { key: 'volume', label: 'Volume', default: 0.7 },
    ],
  },
  {
...
function PedalIcon({
  ...
  switch (type) {
    ...
    case 'nam-amp': {
      ...
      return (
        <svg {...props}>
          <polygon points="8,3 16,3 20,9 12,22 4,9" />
          <polyline points="4,9 20,9" />
          <polyline points="8,3 12,9 16,3" />
...
function createBlock(type: PedalBlockType): PedalBlock {
  ...
  return {
    ...
    enabled: true,
    params,
    namModelId: type === 'nam-amp' ? null : undefined,
  };
}
...
export function GuitarBassView({
  ...
  const handleSelectNamModel = useCallback(
    async (entry: NamModelEntry, blockId: string) => {
      if (!entry.url) return;
  ...
  const selectedColor =
    selectedBlock?.type === 'nam-amp'
      ? (GEM_COLORS[selectedBlock.namModelId ?? ''] ??
        selectedDef?.color ??
        '#60a5fa')
    ...
    <div className="flex h-full">
                ...
                {entries.map((entry) => (
                  ...
                  >
                    <PedalIcon
                      type="nam-amp"
                      size={16}
                      color={GEM_COLORS[entry.id] ?? '#60a5fa'}
                      namModelId={entry.id}
                    />
                    <span
                      className="text-[11px]"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {(() => {
          ...
          const ampColor =
            GEM_COLORS[ampBlock?.namModelId ?? ''] ??
            ampDef?.color ??
            '#60a5fa';
          ...
          return (
            ...
            >
              ...
              <div
                className="flex flex-col items-center shrink-0 pl-4 relative"
                style={{ zIndex: 1 }}
              >
                <div
              ...
              >
                ...
                <EmptySlot
                  chainIdx={
                    pedalBlocks.length === 0
                      ? ampChainIdx >= 0
                        ? ampChainIdx
                        : chain.length
                      : chain.indexOf(pedalBlocks[0])
                  }
                />
              ...
              {/* Sticky amp + out */}
              <div
                className="flex items-center gap-2 shrink-0 pr-4 py-5"
                style={{ zIndex: 1 }}
              >
                {/* Amp tile */}
                {ampBlock && ampDef && (
                  ...
                  >
                    <PedalIcon
                      ...
                      size={28}
                      color={ampColor}
                      namModelId={ampBlock.namModelId ?? undefined}
                    />
                    <div
                      onClick={(e) => {
                        e.stopPropagation();
                        toggleBlock(ampBlock.id);
                      }}
                      className="absolute cursor-pointer rounded-full"
                      style={{
                        bottom: 5,
                        ...
                        width: 7,
                        height: 7,
                        backgroundColor: ampBlock.enabled
                          ? '#22c55e'
                          : 'rgba(255,255,255,0.12)',
                        boxShadow: ampBlock.enabled
                          ? '0 0 4px #22c55e80'
                          : 'none',
                      }}
                    />
