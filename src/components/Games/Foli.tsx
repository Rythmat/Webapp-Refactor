...
type Phase = 'loading' | 'ready' | 'listening' | 'input' | 'success' | 'fail' | 'gameover';
...
class BongoEngine {
  ...
  async init() {
    ...
    const [leftBuf, rightBuf] = await Promise.all([
      fetch(BONGO_LEFT_URL).then((r) => r.arrayBuffer()).then((b) => this.ctx!.decodeAudioData(b)),
      fetch(BONGO_RIGHT_URL).then((r) => r.arrayBuffer()).then((b) => this.ctx!.decodeAudioData(b)),
    ]);
  ...
  async loadMetronome() {
    ...
    const [first, click] = await Promise.all([
      fetch(METRO_FIRST_URL).then((r) => r.arrayBuffer()).then((b) => this.ctx!.decodeAudioData(b)),
      fetch(METRO_URL).then((r) => r.arrayBuffer()).then((b) => this.ctx!.decodeAudioData(b)),
    ]);
  ...
  stopShaker() {
    if (this.shakerGain && this.activeShaker && this.ctx) {
      this.shakerGain.gain.linearRampToValueAtTime(0, this.ctx.currentTime + 0.1);
      const src = this.activeShaker;
      setTimeout(() => { try { src.stop(); } catch { /* ok */ } }, 120);
...
function isDotted(val: NoteValue) { return val.endsWith('.'); }
function isFilledHead(val: NoteValue) { return !['whole', 'half.', 'half'].includes(val); }
function hasNoteStem(val: NoteValue) { return val !== 'whole'; }
...
function restBaseValue(ticks: number): 'whole' | 'half' | 'quarter' | 'eighth' | 'sixteenth' {
  if (ticks >= 1680) return 'whole';
  if (ticks >= 840)  return 'half';
  if (ticks >= 420)  return 'quarter';
  if (ticks >= 180)  return 'eighth';
...
function RestSymbol({ slot }: { slot: RestSlot }) {
  ...
  if (val === 'eighth') {
    return (
      <g>
        ...
        <line x1={x + 1} y1={y - 7} x2={x - 3} y2={y + 5} stroke={c} strokeWidth={1.4} strokeLinecap="round" />
      </g>
    );
  }
  // sixteenth
  return (
    <g>
      ...
      <line x1={x + 1} y1={y - 9} x2={x - 4} y2={y + 5} stroke={c} strokeWidth={1.4} strokeLinecap="round" />
    </g>
  );
}
...
function RhythmNotation({
  ...
  return (
    <svg width={W} height={H} style={{ overflow: 'visible' }}>
      {/* Bar lines */}
      <line x1={PAD_L - 10} y1={barTop} x2={PAD_L - 10} y2={barBot} stroke="rgba(255,255,255,0.25)" strokeWidth={1.5} />
      <line x1={W - PAD_R + 10} y1={barTop} x2={W - PAD_R + 10} y2={barBot} stroke="rgba(255,255,255,0.25)" strokeWidth={1.5} />
      ...
      {sequence.map((note, i) => {
        ...
        return (
          <g key={i} style={{ transition: 'opacity 0.15s' }} opacity={opacity}>
            ...
            {/* Augmentation dot */}
            {dotted && (
              <circle cx={x + HEAD_RX + 4} cy={STAFF_Y - 1} r={1.5} fill={color} />
            )}
...
function FoilDrums({
  ...
  const getFilter = (glow: BongoGlow, pressed: boolean, side: Side): string => {
    const isLit = glow === 'hit' || pressed;
    if (glow === 'success') return 'drop-shadow(0 0 18px rgba(34,197,94,0.7)) drop-shadow(0 0 36px rgba(34,197,94,0.3))';
    if (glow === 'fail') return 'drop-shadow(0 0 18px rgba(248,113,113,0.7)) drop-shadow(0 0 36px rgba(248,113,113,0.3))';
    if (isLit) {
      return side === 'left'
  ...
  const handleLeftDown = () => { if (!disabled) { setPressedLeft(true); onPressLeft(); } };
  const handleRightDown = () => { if (!disabled) { setPressedRight(true); onPressRight(); } };
      ...
      >
        <defs>
          ...
          <clipPath id="foil-clip-right">
            <rect x={CLIP_SPLIT_X} y="0" width={165 - CLIP_SPLIT_X} height="150" />
          </clipPath>
        </defs>
        ...
        <g
          ...
          style={{ cursor: disabled ? 'default' : 'pointer', transition: 'filter 0.15s ease', filter: leftFilter }}
        ...
        >
          <path d={FOIL_PATH} fill={leftFill} fillRule="nonzero" style={{ transition: 'fill 0.15s ease' }} />
        </g>

        {/* Right pillar */}
        <g
          clipPath="url(#foil-clip-right)"
          style={{ cursor: disabled ? 'default' : 'pointer', transition: 'filter 0.15s ease', filter: rightFilter }}
        ...
        >
          <path d={FOIL_PATH} fill={rightFill} fillRule="nonzero" style={{ transition: 'fill 0.15s ease' }} />
        </g>
      </svg>
...
export default function Foli() {
  ...
  const playSequence = useCallback((seq: SequenceNote[], engine: BongoEngine) => {
  ...
  const playRound = useCallback(async (engine: BongoEngine): Promise<boolean> => {
    ...
    const seqSpan = seqTimesMs.length > 1 ? seqTimesMs[seqTimesMs.length - 1] : 1000;
    toleranceRef.current = Math.max(seqSpan * TOLERANCE_RATIO, BEAT_MS * 0.15);
  ...
  const handleHit = useCallback((side: Side) => {
  ...
  return (
    ...
    >
      ...
      {(phase === 'listening' || phase === 'input') && sequence.length > 0 && (
        <RhythmNotation sequence={sequence} matched={matched} barStartTime={barStartTime} />
      )}
      ...
      {phase === 'gameover' && (
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center' }}>
          ...
          >
            Final Score: <span style={{ color: '#ddd6fe', fontWeight: 700, fontFamily: 'monospace' }}>{score}</span>
          </div>
        </div>
      )}
      ...
      {(phase === 'ready' || phase === 'gameover') && (
        <button
          onClick={phase === 'ready' ? startGame : () => { handleReset(); }}
          style={{
            padding: '9px 36px',
            borderRadius: 10,
