...
import { buildApproachNotes, shouldAddApproach } from '../engine/genreGeneration/bassApproach';
...
const SNARE_PATTERNS: Record<string, [number, number][]> = {
  backbeat_gh: [[480, 95], [600, 25], [1440, 95]],
  anticipate:  [[480, 95], [1320, 30], [1440, 95]],
  fill:        [[480, 95], [960, 40], [1080, 30], [1200, 40], [1440, 95]],
  big_fill:    [[480, 95], [840, 35], [960, 50], [1080, 35], [1200, 50],
                [1320, 35], [1440, 95], [1680, 60]],
};
...
function buildHihat(barOffset: number, mode: string): BackingNote[] {
  ...
  if (mode === 'eighth' || mode === 'loose') {
    for (let i = 0; i < 8; i++) {
      ...
      const n   = i === 5 ? 46 : 42;
      const vel = mode === 'loose'
        ? 45 + Math.floor(Math.random() * 30)
        : i % 2 === 0 ? 72 : 52;
      notes.push({ note: n, onset: t, duration: 50, velocity: vel, part: 'drums' });
      if (mode === 'loose' && [1, 3, 7].includes(i) && Math.random() < 0.3) {
        notes.push({ note: 42, onset: barOffset + i * 240 + 120 + jitter(0, 4),
          duration: 40, velocity: 32, part: 'drums' });
      }
    }
  } else if (mode === 'double4') {
    for (let i = 0; i < 6; i++) {
      notes.push({ note: i === 5 ? 46 : 42,
        onset: barOffset + i * 240 + jitter(0, 4),
        duration: 50, velocity: i % 2 === 0 ? 72 : 52, part: 'drums' });
    }
    [1440, 1560, 1680, 1800].forEach((t0) => {
      notes.push({ note: 42, onset: barOffset + t0 + jitter(0, 3),
        duration: 40, velocity: t0 === 1440 ? 65 : 50, part: 'drums' });
    });
  }
  return notes;
}
...
function buildDrumPattern(bars: number): BackingNote[] {
  ...
  for (let bar = 0; bar < bars; bar++) {
    ...
    (SNARE_PATTERNS[snareName] ?? []).forEach(([t0, vel]) => {
      notes.push({ note: 38, onset: o + t0 + jitter(0, 4),
        duration: 80, velocity: vel, part: 'drums' });
    });
...
function H(): number { return Math.floor(Math.random() * 7); } // 0-6 ticks (H5)
...
function buildDrumPattern_02(bars: number): BackingNote[] {
  ...
  const bar1: [number, number, number][] = [
    [0,KICK,104],[0,HH_CL,104],[240,HH_CL,104],
    [480,SNARE,104],[480,HH_CL,104],[720,HH_CL,104],
    [840,KICK,104],[960,HH_CL,104],
    [1200,KICK,104],[1200,HH_OP,104],
    [1440,SNARE,104],[1440,HH_CL,104],[1680,HH_CL,104],[1800,KICK,104],
  ];
  const bar2: [number, number, number][] = [
    [0,KICK,104],[0,HH_CL,104],[240,HH_CL,104],
    [480,SNARE,104],[480,HH_CL,104],[720,HH_CL,104],
    [840,KICK,104],[960,HH_CL,104],
    [1200,KICK,104],[1200,HH_CL,104],
    [1440,SNARE,104],[1440,HH_CL,104],[1560,KICK,104],[1560,HH_OP,104],
  ];
  const cell = [bar1, bar2];
  for (let rep = 0; rep < bars; rep++) {
    const o = rep * BAR;
    cell[rep % 2].forEach(([t, n, v]) => {
      notes.push({ note: n, onset: o + t + H(), duration: n === HH_OP ? 200 : n === KICK || n === SNARE ? 100 : 50, velocity: v, part: 'drums' });
    });
  }
  return notes;
}
...
function buildDrumPattern_03(bars: number): BackingNote[] {
  ...
  const BAR = 1920;
  const bar1: [number, number, number][] = [
    [0,KICK,104],[0,HH_CL,104],[120,HH_CL,104],
    [240,KICK,104],[240,HH_CL,104],[360,HH_CL,104],
    [480,SNARE,104],[480,HH_CL,104],[600,HH_CL,104],[720,HH_CL,104],
    [840,KICK,104],[840,HH_CL,104],[960,HH_CL,104],[1080,HH_CL,104],
    [1200,KICK,104],[1200,HH_CL,104],[1320,HH_CL,104],
    [1440,SNARE,104],[1440,HH_CL,104],[1560,HH_CL,104],[1680,HH_CL,104],
    [1800,KICK,104],[1800,HH_OP,104],
  ];
  const bar2: [number, number, number][] = [
    [0,HH_CL,104],[120,KICK,104],[120,HH_CL,104],
    [240,KICK,104],[240,HH_CL,104],[360,HH_CL,104],
    [480,SNARE,104],[480,HH_CL,104],[600,HH_CL,104],[720,HH_CL,104],
    [840,KICK,104],[840,HH_OP,104],[960,HH_CL,104],
    [1080,KICK,104],[1080,HH_CL,104],[1200,KICK,104],[1200,HH_CL,104],[1320,HH_CL,104],
    [1440,SNARE,104],[1440,HH_CL,104],[1560,KICK,104],[1560,HH_OP,104],
  ];
  const cell = [bar1, bar2];
  for (let rep = 0; rep < bars; rep++) {
...
function buildDrumPattern_04(bars: number): BackingNote[] {
  ...
  const BAR = 1920;
  const bar1: [number, number, number][] = [
    [0,KICK,104],[0,HH_OP,104],
    [480,KICK,104],[480,SNARE,104],[600,HH_CL,104],[720,HH_CL,104],
    [840,KICK,104],[840,HH_CL,104],[960,KICK,104],[960,HH_CL,104],
    [1080,HH_CL,104],[1200,HH_CL,104],[1320,HH_CL,104],
    [1440,SNARE,104],[1560,HH_CL,104],[1680,HH_CL,104],
    [1800,KICK,104],[1800,HH_CL,104],
  ];
  const bar2: [number, number, number][] = [
    [0,KICK,104],[0,HH_CL,104],[120,HH_CL,104],[240,HH_CL,104],
    [480,SNARE,104],[600,HH_CL,104],[720,HH_CL,104],
    [840,KICK,104],[840,HH_OP,104],[960,KICK,104],[960,HH_CL,104],
    [1080,HH_CL,104],[1200,KICK,104],[1200,HH_CL,104],[1320,HH_CL,104],
    [1440,SNARE,104],[1440,HH_CL,104],[1560,KICK,104],[1560,HH_CL,104],
    [1680,HH_CL,104],[1800,HH_CL,104],
  ];
  const cell = [bar1, bar2];
  for (let rep = 0; rep < bars; rep++) {
    const o = rep * BAR;
...
function buildDrumPattern_05(bars: number): BackingNote[] {
  ...
  const BAR = 1920;
  const bar1: [number, number, number][] = [
    [0,KICK,104],[0,HH_CL,104],[240,KICK,104],[240,HH_CL,104],
    [480,SNARE,104],[480,HH_CL,104],[720,HH_CL,104],
    [840,SNARE,104],[960,HH_CL,104],
    [1080,SNARE,66], // ghost snare
    [1200,KICK,104],[1200,HH_CL,104],[1440,HH_CL,104],
    [1680,SNARE,104],[1680,HH_CL,104],
  ];
  const bar2: [number, number, number][] = [
    [0,HH_CL,104],
    [120,SNARE,66], // ghost snare
    [240,KICK,104],[240,HH_CL,104],
    [480,SNARE,104],[480,HH_CL,104],[720,HH_CL,104],
    [840,SNARE,66], // ghost snare
    [960,HH_CL,104],
    [1080,SNARE,66], // ghost snare
    [1200,KICK,104],[1200,HH_CL,104],
    [1440,SNARE,104],[1440,HH_CL,104],
    [1560,KICK,104],[1680,HH_OP,104],
  ];
  const cell = [bar1, bar2];
  for (let rep = 0; rep < bars; rep++) {
    const o = rep * BAR;
...
function buildDrumPattern_06(bars: number): BackingNote[] {
  ...
  const BAR = 1920;
  const bar1: [number, number, number][] = [
    [0,KICK,104],[0,HH_CL,104],
    [120,SNARE,66],[120,HH_CL,66],
    [240,HH_CL,104],
    [360,KICK,104],[360,HH_CL,66],
    [480,SNARE,104],[480,HH_CL,104],
    [600,HH_CL,66],[720,HH_CL,104],
    [840,SNARE,66],[840,HH_CL,66],
    [840+DOUBLE_OFFSET,SNARE,66], // 32nd flam
    [960,HH_CL,104],
    [1080,SNARE,66],[1080,HH_CL,66],
    [1200,KICK,104],[1200,HH_CL,104],
    [1320,KICK,104],[1320,HH_CL,66],
    [1440,SNARE,104],[1440,HH_CL,104],
    [1560,HH_CL,66],[1680,HH_CL,104],
    [1800,SNARE,66],[1800,HH_CL,66],
  ...
  const bar2: [number, number, number][] = [
    ...
    [1320,HH_CL,66],
    [1440,SNARE,104],[1440,HH_CL,104],
    [1560,KICK,104],[1560,HH_CL,66],
    [1680,SNARE,66],[1680,HH_CL,104],
    [1800,SNARE,66],[1800,HH_CL,66],
  ];
  const cell = [bar1, bar2];
  for (let rep = 0; rep < bars; rep++) {
...
type GrooveId = 'groove_funk_01' | 'groove_funk_02' | 'groove_funk_03'
  | 'groove_funk_04' | 'groove_funk_05' | 'groove_funk_06';

function getGrooveForStyleRef(styleRef?: string): GrooveId {
  switch (styleRef) {
    case 'l1a':  return 'groove_funk_01';
    case 'l1b':  return 'groove_funk_02';
    case 'l2a':  return 'groove_funk_05';
    case 'l2b':  return 'groove_funk_03';
    case 'l3a':  return 'groove_funk_04';
    case 'l3b':  return 'groove_funk_06';
    default:     return 'groove_funk_01';
  }
}

function buildDrumPatternForGroove(bars: number, grooveId: GrooveId = 'groove_funk_01'): BackingNote[] {
  switch (grooveId) {
    case 'groove_funk_02': return buildDrumPattern_02(bars);
    case 'groove_funk_03': return buildDrumPattern_03(bars);
    case 'groove_funk_04': return buildDrumPattern_04(bars);
    case 'groove_funk_05': return buildDrumPattern_05(bars);
    case 'groove_funk_06': return buildDrumPattern_06(bars);
    case 'groove_funk_01':
    default:               return buildDrumPattern(bars);
  }
}
...
function buildBassPattern(
  bars: number, keyRoot: number,
  level: number, chordSymbols: string[] | undefined,
): BackingNote[] {
  const bassNotes = deriveBassNotes(chordSymbols, keyRoot, bars);
  console.log('[buildBassPattern] chordSymbols:', chordSymbols, 'keyRoot:', keyRoot, 'bassNotes:', bassNotes);
  const notes: BackingNote[] = [];
  for (let bar = 0; bar < bars; bar++) {
    const o        = bar * 1920;
    ...
    const push = (note: number, onset: number, dur: number, vel: number) =>
      notes.push({ note, onset: o + onset + jitter(0, 5),
        duration: dur, velocity: vel, part: 'bass' });
    ...
    if (level === 1) {
      ...
      if (shouldAddApproach(bar, bars)) {
        ...
        approachNotes.forEach(a =>
          notes.push({ note: a.midi, onset: a.onset,
            duration: a.duration, velocity: 75, part: 'bass' }));
      }
    } else {
      push(root,  0,    220, 96);
      push(oct,   240,  120, 85);
...
const APPROACH_CELLS: Record<string, CellStep[]> = {
  P1: [[0, 'G', E8], [E8, 'A', S16], [E8 + S16, 'G', S16]],
  P2: [[0, 'A', S16], [S16, 'G', S16], [S16 * 3, 'G', S16]],
  P3: [[0, 'AA', E8], [E8, 'A2', S16], [E8 + S16, 'G', S16]],
  P4: [[0, 'AA', E8], [E8, 'A2', E8], [E8 * 2, 'G', S16], [E8 * 2 + S16, 'G', S16]],
};
...
interface ChordVoicings { G: number[]; A: number[]; A2: number[]; AA: number[] }

function buildVoicings(keyRoot: number): ChordVoicings {
  let b7 = keyRoot + 10;
  while (b7 < 57) b7 += 12;
  while (b7 > 67) b7 -= 12;
  const b3 = b7 + 5, fif = b7 + 9;
  return {
    G:  [b7,     b3,     fif    ],
    A:  [b7 - 1, b3 - 1, fif - 1],
    A2: [b7 + 1, b3 + 1, fif + 1],
...
function makeStabs(
  barOffset: number, pitches: number[],
  onsets: number[], dur: number, vel: number,
): BackingNote[] {
  const notes: BackingNote[] = [];
  onsets.forEach((t0) => {
    const t = barOffset + t0 + jitter(-10, 10);
    pitches.forEach((p) =>
      notes.push({ note: p, onset: Math.max(0, t),
        duration: dur, velocity: vel + jitter(0, 6), part: 'chords' }));
  });
  return notes;
}
...
function makeApproachBar(o: number, v: ChordVoicings, cellId?: string): BackingNote[] {
  const id     = cellId ?? (Math.random() < 0.5 ? 'P1' : 'P2');
  const starts = CELL_VALID_STARTS[id] ?? [];
  const pool   = starts.filter((t) => t >= 720).length > 0 && Math.random() < 0.7
    ? starts.filter((t) => t >= 720) : starts;
  const start  = pool[Math.floor(Math.random() * pool.length)];
  const notes: BackingNote[] = [];
  (APPROACH_CELLS[id] ?? []).forEach(([rel, role, dur]) => {
    const t = o + start + rel;
    v[role].forEach((p) =>
      notes.push({ note: p, onset: t, duration: dur,
        velocity: (role === 'G' ? 72 : 68) + jitter(0, 6), part: 'chords' }));
  });
  PRIMARY_STAB_POSITIONS.forEach((t0) => {
    if (Math.abs(t0 - start) < 200) return;
    const t = o + t0 + jitter(-10, 10);
    v.G.forEach((p) =>
      notes.push({ note: p, onset: Math.max(0, t), duration: 120,
        velocity: 72 + jitter(0, 6), part: 'chords' }));
  });
  return notes;
}
...
function makeCadenceBar(o: number, v: ChordVoicings): BackingNote[] {
  if (Math.random() < 0.33) {
    ...
    (APPROACH_CELLS[id] ?? []).forEach(([rel, role, dur]) => {
      v[role].forEach((p) =>
        notes.push({ note: p, onset: o + 960 + rel, duration: dur,
          velocity: (role === 'G' ? 72 : 68) + jitter(0, 6), part: 'chords' }));
    });
...
function buildChordPattern(
  ...
  chordsByBar: number[][], keyRoot: number,
): BackingNote[] {
  const notes: BackingNote[] = [];
  ...
  if (level === 3) {
    for (let bar = 0; bar < bars; bar++) {
      ...
      (chordsByBar[bar % chordsByBar.length] ?? []).forEach((midi) =>
        notes.push({ note: midi, onset: o, duration: 1800,
          velocity: 68, part: 'chords' }));
    }
  ...
  for (let bar = 0; bar < Math.min(bars, 16); bar++) {
    ...
    if (phrasePos === 3) {
      ...
      notes.push(...(isSecondPass && variationBar === 0
        ? makeApproachBar(o, v) : makePlainBar(o, v)));
    } else if (phrasePos === 1) {
      notes.push(...(isSecondPass && variationBar === 1
        ? makePlainBar(o, v) : makeApproachBar(o, v, bar2CellId)));
    } else {
      notes.push(...(isSecondPass && variationBar === 2
        ? makeApproachBar(o, v) : makePlainBar(o, v)));
    }
  }
  return removeChordStutters(notes);
...
export function useBackingTrack(tempo: number) {
    ...
    if (!bassSynthRef.current) {
      bassSynthRef.current = new Tone.MonoSynth({
        ...
        filterEnvelope: {
          ...
          release: 0.2, baseFrequency: 200, octaves: 2,
        },
        volume: -6,
      }).toDestination();
    }
  ...
  const initSF2 = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      try {
        ...
        await drumEngineRef.current.init(ctx, drumBridgeRef.current.input as unknown as AudioNode);
      } catch (drumErr) {
      ...
      try {
        ...
        console.warn('[useBackingTrack] SF2 also failed, using Tone.js synths', sf2Err);
        sf2LoadingRef.current = false;
  ...
  const disposeAll = useCallback(() => {
    Object.values(partsRef.current).forEach((part) => {
      if (part) { try { part.stop(0); } catch { /* ignore negative time rounding */ } part.dispose(); }
    });
    partsRef.current = {};
  ...
  const startBacking = useCallback(async (
    ...
    if (engineGenerates.includes('drums')) {
      const grooveId = (step.grooveId ?? getGrooveForStyleRef(styleRef)) as GrooveId;
      allNotes.push(...buildDrumPatternForGroove(BACKING_BARS, grooveId));
    }
    if (engineGenerates.includes('bass'))
      allNotes.push(...buildBassPattern(BACKING_BARS, keyRoot, level, step.chordSymbols));
    if (engineGenerates.includes('chords'))
      allNotes.push(...buildChordPattern(BACKING_BARS, level, chordsByBar, keyRoot));
    ...
    const beat1Notes = allNotes.filter(n => n.onset < 120 && n.part !== 'drums');
    ...
    if (import.meta.env.DEV) {
      const kicks = allNotes.filter(n => n.note === 36)
        .map(n => n.onset).sort((a, b) => a - b);
      let maxRun = 1, run = 1;
      ...
      console.log(`[backing] ${allNotes.length} notes, max kick run: ${maxRun}`);
    }

    // ── Drums ─────────────────────────────────────────────────────────────
    const drumEvents = allNotes.filter(n => n.part === 'drums')
      .map(n => ({ time: n.onset * spt, note: n.note, velocity: n.velocity }));
      ...
      const drumPart = new Tone.Part(
        (time, value: { note: number; velocity: number }) => {
          if (useReal && drumEngineRef.current) {
            ...
            if      (value.note === 36) kickSynthRef.current?.triggerAttackRelease('C1', '8n', time, vel);
            else if (value.note === 38) snareSynthRef.current?.triggerAttackRelease('16n', time, vel);
            ...
            else if (value.note === 46) hihatSynthRef.current?.triggerAttackRelease('16n', time, vel * 0.8);
          }
        },
        drumEvents,
      );
    ...
    const rawBass = allNotes.filter(n => n.part === 'bass')
    ...
    const bassEvents = rawBass.map((n, i) => {
      ...
      const isPickup = next != null && (next.onset - n.onset) <= IMMEDIATE_TICKS;
        ...
        (time, value: {
          ...
          if (useReal && bassSamplerRef.current) {
            ...
            bassSamplerRef.current.triggerAttackRelease(
              noteName, value.durationSec, time, value.velocity / 127,
            );
    ...
    // ── Chords ────────────────────────────────────────────────────────────
    const chordEvents = allNotes.filter(n => n.part === 'chords')
      .map(n => ({
        time:        n.onset * spt,
        note:        n.note,
        velocity:    n.velocity,
    ...
    if (chordEvents.length) {
      const chordPart = new Tone.Part(
        (time, value: {
          note: number; velocity: number; durationSec: number; durationMs: number;
        }) => {
          if (useReal) {
