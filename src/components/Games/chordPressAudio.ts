// ── Chord Press audio ────────────────────────────────────────────────────────
// Plays through the shared Jam Room SpessaSynth using the GM "Acoustic Grand
...
/** Play a single MIDI note on the Acoustic Grand Piano. */
export async function playGrandPianoNote(midi: number): Promise<void> {
  try {
    await initJamSynth();
    // Re-assert Acoustic Grand Piano every time in case another feature (e.g.
    // the Chroma game's Crystal sound) changed the program on this channel.
    jamProgramChange(PIANO_CHANNEL, GRAND_PIANO_PROGRAM);
