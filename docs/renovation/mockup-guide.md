...
| Topic | Decision |
|---|---|
| **Default structure** | **Standards-Based is the default Process for every Day, always.** It's a new *name* for Atlas's fundamental curricular structure, not a new concept. |
...
| Action | Today | Mock |
|---|---|---|
| **Edit** | Name + description only | Add start/end dates, days of week, location, thumbnail upload *(needs backend, C5)* |
...
The *content* is hierarchical (a Unit contains Weeks and Days; a Project links CLOs), but the *navigation* is not. A teacher should never need to open one section to discover another exists.
...
| **Learn-to-Apply** | CONNECT/REGULATE → Connect · ARTICULATE CONTEXT → Connect · TEACHER MODEL/ASSIST → **Observe** · BUILD SKILL → Practice · APPLY SKILL → Create *(or Present for ensembles?)* · RESPOND/RESET → Respond |
...
- Do "Artifacts" mean *student work to collect*, or *materials to reuse*?
...
4. **Picture**: from the Atlas library first; upload from device later *(needs backend)*.
5. **Text**
