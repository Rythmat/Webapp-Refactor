import type { LessonProgressEntry, LessonProgressResponse, ProgressSummaryResponse } from './types';
...
export function reconcileProgressSummary(
  ...
  return { lessons: [...map.values()].sort((a, b) => ts(b.updatedAt) - ts(a.updatedAt)) };
}
...
export function reconcileLessonProgress(
  ...
  Object.entries(serverValue.progressByActivityInstanceId).forEach(([id, serverEntry]) => {
    const localEntry = merged[id];
    if (!localEntry || ts(serverEntry.updatedAt) >= ts(localEntry.updatedAt)) {
  ...
  const localCurrentUpdated = localCurrent ? merged[localCurrent]?.updatedAt : undefined;
