...
const HARMONIC_DATA: HarmonicInfo[] = Array.from({ length: HARMONICS_COUNT }, (_, i) => {
  const n = i + 1;
  const freq = FUNDAMENTAL_FREQ * n;
  const intervals = [
    'Unison', 'Octave', 'P5 (+oct)', 'Octave 2', 'M3 (+2oct)', 'P5 (+2oct)',
    'm7 (+2oct)', 'Octave 3', 'M2 (+3oct)', 'M3 (+3oct)', 'Tritone (+3oct)',
    'P5 (+3oct)', 'm6 (+3oct)', 'm7 (+3oct)', 'M7 (+3oct)', 'Octave 4',
  ];
...
const HARMONIC_COLORS = [
  '#a78bfa', '#38bdf8', '#34d399', '#fbbf24', '#f97316', '#ef4444',
  '#ec4899', '#8b5cf6', '#06b6d4', '#10b981', '#eab308', '#f59e0b',
...
class HarmonicEngine {
  ...
  stop() {
    if (this.activeOsc && this.activeGain) {
      this.activeGain.gain.linearRampToValueAtTime(0, this.ctx.currentTime + 0.05);
      const osc = this.activeOsc;
      setTimeout(() => { try { osc.stop(); } catch { /* ok */ } }, 60);
      this.activeOsc = null;
...
function StringVisualization({
  ...
  for (let i = 0; i <= resolution; i++) {
    ...
    const y = midY + amplitude * Math.sin(harmonic * Math.PI * t) * Math.sin(phase * 3);
    points.push(`${x},${y}`);
  }
  ...
  return (
    <svg width={width} height={height} className="block">
      {/* Bridge ends */}
      <rect x={padding - 4} y={midY - 15} width={8} height={30} rx={2} fill="#3f3f46" />
      <rect x={padding + stringWidth - 4} y={midY - 15} width={8} height={30} rx={2} fill="#3f3f46" />
      ...
      <path
        ...
        style={isAnimating ? { filter: `drop-shadow(0 0 6px ${color})` } : undefined}
      />

      {/* Nodes */}
      {showNodes && nodes.map((x, i) => (
        <g key={i}>
          <circle
            cx={x}
            cy={midY}
          ...
          />
          {isAnimating && (
            <circle cx={x} cy={midY} r={10} fill="none" stroke={color} strokeWidth={0.5} opacity={0.4} />
          )}
        </g>
      ))}

      {/* Antinode labels */}
      {isAnimating && harmonic <= 8 && Array.from({ length: harmonic }, (_, i) => {
...
export default function HarmonicStrings({ onComplete }: HarmonicStringsProps) {
  ...
  const playHarmonic = useCallback((n: number) => {
    const engine = getEngine();
    ...
    <div className="flex flex-col bg-[#09090b] rounded-2xl overflow-hidden border border-zinc-800">
      ...
      {mode === 'explore' && info && (
        <div className="px-6 pb-2 flex items-center gap-6">
          <div>
            <span className="text-[10px] text-zinc-600 uppercase tracking-wider">Harmonic</span>
          ...
          <div>
            ...
            <div className="text-sm text-zinc-300 font-mono">{info.frequency.toFixed(1)} Hz</div>
          </div>
          <div>
            <span className="text-[10px] text-zinc-600 uppercase tracking-wider">Interval</span>
            <div className="text-sm text-zinc-300">{info.interval}</div>
          </div>
          {info.cents !== 0 && (
            <div>
              <span className="text-[10px] text-zinc-600 uppercase tracking-wider">ET Deviation</span>
        ...
        <div className="px-4 pb-4">
          <div className="flex items-end gap-0.5 h-16">
            {HARMONIC_DATA.map((h) => {
              ...
              const color = HARMONIC_COLORS[(h.number - 1) % HARMONIC_COLORS.length];
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderLeft: active ? `1px solid ${color}` : '1px solid rgba(255,255,255,0.05)',
                    borderRight: active ? `1px solid ${color}` : '1px solid rgba(255,255,255,0.05)',
                    borderBottom: active ? `1px solid ${color}` : '1px solid rgba(255,255,255,0.05)',
                  }}
                >
                  {h.number}
                </button>
        ...
        <div className="px-6 pb-4">
          {quizQuestion === null ? (
            ...
            <div className="flex flex-col items-center gap-4">
              <p className="text-sm text-zinc-400">
                What interval is harmonic #{quizQuestion.number} above the fundamental?
              </p>
            ...
            <div className="flex flex-col items-center gap-3">
              <div
                ...
                style={{ color: quizResult === 'correct' ? '#34d399' : '#ef4444' }}
              >
                {quizResult === 'correct' ? 'Correct!' : `Wrong — it was ${quizQuestion.interval}`}
              </div>
              <p className="text-xs text-zinc-500">
                Harmonic {quizQuestion.number}: {quizQuestion.frequency.toFixed(1)} Hz
                {quizQuestion.cents !== 0 && ` (${quizQuestion.cents > 0 ? '+' : ''}${quizQuestion.cents}¢ from ET)`}
              </p>
              <button
                onClick={newQuiz}
