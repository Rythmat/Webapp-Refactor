import type { MidiSequence, OrchestrateConfig } from '../types';
...
export function orchestrate(config: OrchestrateConfig): Map<InstrumentChannel, MidiSequence> {
  const result = new Map<InstrumentChannel, MidiSequence>();

  const chordPattern = CHORD_RHYTHMS[config.rhythmName] ?? CHORD_RHYTHMS['Quarters'];
  const melodyPattern = MELODY_RHYTHMS[config.rhythmName] ?? MELODY_RHYTHMS['Quarters'];

  if (config.enableDrums) {
    result.set(InstrumentChannel.Drums, generateDrumMidi({
  ...
  if (config.enableBass) {
    result.set(InstrumentChannel.Bass, generateBassMidi({
  ...
  if (config.enablePad) {
    result.set(InstrumentChannel.Pad, generatePadMidi({
      chordSeq: config.chordSeq,
      stringSeq: config.stringSeq,
      rhythmName: config.rhythmName,
      swing: config.swing,
  ...
  if (config.enableChords) {
    result.set(InstrumentChannel.Chords, generateChordMidi({
      chordSeq: config.chordSeq,
      stringSeq: config.stringSeq,
  ...
  if (config.enableMelody) {
    result.set(InstrumentChannel.Melody, generateMelodyMidi({
      chordSeq: config.chordSeq,
      stringSeq: config.stringSeq,
      root: config.root,
