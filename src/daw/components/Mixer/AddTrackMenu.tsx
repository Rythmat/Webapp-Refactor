...
function GuitarPickIcon({ size = 20 }: { size?: number }) {
  return (
...
const TRACK_TEMPLATES: TrackTemplate[] = [
  { label: 'Vocal', icon: <Mic size={ICON_SIZE} strokeWidth={2} />, color: '#ef4444', trackType: 'audio', instrument: 'vocal-fx' },
  { label: 'Instruments', icon: <Music size={ICON_SIZE} strokeWidth={2} />, color: '#22c55e', trackType: 'midi', instrument: 'soundfont' },
  { label: 'Synth', icon: <KeyboardMusic size={ICON_SIZE} strokeWidth={2} />, color: '#8b5cf6', trackType: 'midi', instrument: 'oracle-synth' },
  { label: 'Organ', icon: <Piano size={ICON_SIZE} strokeWidth={2} />, color: '#FFB347', trackType: 'midi', instrument: 'tonewheel-organ' },
  { label: 'Drums', icon: <Drum size={ICON_SIZE} strokeWidth={2} />, color: '#f59e0b', trackType: 'midi', instrument: 'drum-machine' },
  { label: 'Guitar', icon: <GuitarPickIcon size={ICON_SIZE} />, color: '#3b82f6', trackType: 'audio', instrument: 'guitar-fx' },
  { label: 'Bass', icon: <GuitarPickIcon size={ICON_SIZE} />, color: '#8b5cf6', trackType: 'audio', instrument: 'bass-fx' },
  { label: 'Import', icon: <FileUp size={ICON_SIZE} strokeWidth={2} />, color: '#6b7280', trackType: 'midi', instrument: 'none' },
];
...
export function AddTrackMenu({ onClose }: AddTrackMenuProps) {
  ...
  return (
    <div
      className="flex items-center gap-1 rounded-2xl px-3 py-2 mx-auto glass-panel"
      style={{
    ...
    >
      {TRACK_TEMPLATES.map((t) => (
        <button
          ...
          className="flex flex-col items-center gap-1 rounded-xl px-3 py-2 transition-colors hover:bg-white/[0.06] active:scale-95 cursor-pointer"
          style={{ minWidth: 64 }}
        >
