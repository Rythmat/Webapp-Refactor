...
export function GateCurve({ threshold, range, width = 140, height = 80 }: GateCurveProps) {
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    const cmd = i === 0 ? 'M' : 'L';
    points.push(`${cmd}${dbToX(inputDb).toFixed(1)} ${dbToY(outputDb).toFixed(1)}`);
  }
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="shrink-0">
      ...
      {/* Grid */}
      {gridDbs.map((db) => (
        <line key={`h-${db}`} x1={PADDING.left} y1={dbToY(db)} x2={width - PADDING.right} y2={dbToY(db)}
          stroke="rgba(255, 255, 255, 0.08)" strokeWidth={0.5}
          strokeDasharray="1 4" />
      ))}
      {gridDbs.map((db) => (
        <line key={`v-${db}`} x1={dbToX(db)} y1={PADDING.top} x2={dbToX(db)} y2={height - PADDING.bottom}
          stroke="rgba(255, 255, 255, 0.08)" strokeWidth={0.5}
          strokeDasharray="1 4" />
      ))}

      {/* Unity line */}
      <path d={unityPath} fill="none" stroke="rgba(255, 255, 255, 0.12)" strokeWidth={0.75} strokeDasharray="3 3" />

      {/* Threshold marker */}
      <line x1={threshX} y1={PADDING.top} x2={threshX} y2={height - PADDING.bottom}
        stroke="rgba(255, 180, 100, 0.25)" strokeWidth={0.75} strokeDasharray="2 2" />
      ...
      <text x={width - PADDING.right} y={height - 2} fill="rgba(255,255,255,0.25)" fontSize={7}
        fontFamily="system-ui" textAnchor="end">0dB</text>
    </svg>
  );
}
