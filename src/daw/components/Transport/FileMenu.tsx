...
import { importMidiFile, exportMidiFile, downloadMidiBlob } from '@/daw/midi/MidiFileIO';
...
export function FileMenu() {
  ...
  const handleExport = useCallback(() => {
    const state = useStore.getState();
    const sequences = new Map<number, { ticksPerQuarterNote: number; trackName: string; events: MidiNoteEvent[] }>();
    ...
    <>
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <button
            className="flex items-center gap-1 px-2 h-7 rounded-md text-[11px] font-medium transition-colors hover:bg-white/5 cursor-pointer"
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content
            className="min-w-[160px] rounded-lg p-1 z-50 shadow-lg"
            style={{
          ...
          >
            <DropdownMenu.Item
              className="flex items-center gap-2 px-3 py-2 text-xs rounded-md outline-none cursor-pointer transition-colors hover:bg-white/5"
