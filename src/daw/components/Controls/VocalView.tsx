...
const PEDAL_CATALOG: PedalDef[] = [
  ...
  },
  {
    ...
    color: '#4ade80',
    params: [
      { key: 'drive', label: 'Drive', default: 0.5 },
      { key: 'tone', label: 'Tone', default: 0.5 },
      { key: 'volume', label: 'Volume', default: 0.7 },
    ],
  },
  {
    type: 'compressor',
    ...
    params: [
      ...
      { key: 'ratio', label: 'Ratio', default: 0.3 },
      { key: 'attack', label: 'Attack', default: 0.2 },
      { key: 'release', label: 'Release', default: 0.5 },
    ],
  },
  {
    type: 'chorus',
    ...
    color: '#a78bfa',
    params: [
      { key: 'rate', label: 'Rate', default: 0.3 },
  ...
  },
  {
    ...
    color: '#c084fc',
    params: [
      { key: 'rateMode', label: 'Mode', default: 0 },
      { key: 'rate', label: 'Hz', default: 0.3 },
      { key: 'rateIdx', label: 'Rate', default: 10 },
      { key: 'depth', label: 'Depth', default: 0.5 },
      { key: 'mix', label: 'Mix', default: 0.5 },
    ],
  },
  ...
  },
  {
    ...
    params: [
      ...
      { key: 'feedback', label: 'Feedback', default: 0.3 },
      { key: 'mix', label: 'Mix', default: 0.4 },
    ],
  },
  {
    type: 'reverb',
    ...
    color: '#38bdf8',
    params: [
      { key: 'size', label: 'Size', default: 0.5 },
      { key: 'decay', label: 'Decay', default: 0.5 },
      { key: 'mix', label: 'Mix', default: 0.3 },
    ],
  },
  {
    type: 'eq',
    ...
    color: '#34d399',
    params: [
      { key: 'low', label: 'Low', default: 0.5 },
      { key: 'mid', label: 'Mid', default: 0.5 },
      { key: 'high', label: 'High', default: 0.5 },
    ],
  },
  {
    type: 'wah',
...
function PedalIcon({
  ...
  switch (type) {
    ...
    case 'compressor':
      return (
        <svg {...props}>
          <polyline points="4,18 10,12 14,14 20,6" />
    ...
    case 'chorus':
      return (
        <svg {...props}>
          ...
          <path d="M2,14 Q6,8 10,14 Q14,20 18,14 Q20,11 22,14" opacity="0.5" />
        </svg>
      );
    case 'phaser':
      return (
        <svg {...props}>
          <path d="M2,12 C5,4 8,20 12,12 C16,4 19,20 22,12" />
        </svg>
      );
    case 'flanger':
      return (
    ...
    case 'delay':
      return (
        <svg {...props}>
          ...
          <circle cx="13" cy="12" r="1.8" opacity="0.6" />
          <circle cx="19" cy="12" r="1.2" opacity="0.3" />
        </svg>
      );
    case 'reverb':
      return (
        <svg {...props}>
          <path d="M12,6 Q18,6 18,12 Q18,18 12,18" fill="none" />
          <path d="M12,9 Q15,9 15,12 Q15,15 12,15" fill="none" />
    ...
    case 'wah':
      return (
        <svg {...props}>
          <path d="M4,18 L10,6 L20,6" />
          <path d="M4,18 L20,18 L20,6" />
        </svg>
      );
  }
}
...
export function VocalView({ trackId }: { trackId: string }) {
  ...
  const storedChain = useStore(
    (s) => s.tracks.find((t) => t.id === trackId)?.vocalChain,
  );
  ...
  useEffect(() => {
    useStore.getState().setVocalChain(
      trackId,
      chain.map((b) => ({
        type: b.type,
        enabled: b.enabled,
  ...
  const pcBlockId =
    chain.find((b) => b.type === 'pitch-correction')?.id ?? null;
  useEffect(() => {
      ...
      if (now - lastUpdate >= 66) {
        ...
        if (adapter && meterBarRef.current) {
          ...
          meterBarRef.current.style.backgroundColor =
            pct > 90 ? '#ef4444' : 'var(--color-accent)';
        }
        lastUpdate = now;
      }
  ...
  const configsMatch = (
    a: AudioInputChannel | null,
    b: AudioInputChannel | null,
  ): boolean => {
    if (a === null && b === null) return true;
    if (a === null || b === null) return false;
    if (a.mode !== b.mode) return false;
    if (a.mode === 'mono' && b.mode === 'mono') return a.channel === b.channel;
    if (a.mode === 'stereo' && b.mode === 'stereo')
  ...
  const pitchActiveNotes = pitchBlock
    ? (pitchBlock.params.activeNotes ??
      scaleToNotesBitmask(
        pitchBlock.params.rootNote ?? 0,
        pitchBlock.params.scaleType ?? 0,
      ))
    : 4095;
  ...
  return (
    <div className="flex h-full">
      {/* ── Input Section ──────────────────────────────────── */}
      <div
        className="w-[240px] shrink-0 p-4 flex flex-col gap-3 border-r"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {(() => {
          ...
          const EmptySlot = ({ chainIdx }: { chainIdx: number }) => (
            <div
              onClick={() =>
                setInsertSlotIndex((prev) =>
                  prev === chainIdx ? null : chainIdx,
                )
              }
              onDragOver={(e) => {
                e.preventDefault();
                setDragOverSlot(chainIdx);
              }}
              ...
              style={{
                ...
                height: 68,
                border:
                  insertSlotIndex === chainIdx || dragOverSlot === chainIdx
                    ? '2px solid var(--color-accent)'
                    : '2px dashed rgba(255,255,255,0.15)',
                backgroundColor:
                  dragOverSlot === chainIdx
          ...
          return (
            ...
            >
              ...
              {/* In: Mic icon + label */}
              <div
                className="flex flex-col items-center gap-0.5 shrink-0 pl-4 relative"
                style={{ zIndex: 1 }}
              >
                <Mic
                  size={22}
              ...
              <div
                className="flex-1 flex items-center justify-center gap-2 px-3 py-5 overflow-x-auto"
                style={{ zIndex: 1 }}
              >
                {/* First empty slot */}
                ...
                {chain.map((block) => {
                  ...
                  const blockColor =
                    block.type === 'pitch-correction'
                      ? (block.params.rootNote ?? -1) === -1
                        ? '#ffffff'
                        : rootNoteColor(block.params.rootNote ?? 0)
                      : def.color;
              ...
              {/* Out label */}
              <div
                className="flex items-center shrink-0 pr-4 py-5"
                style={{ zIndex: 1 }}
              >
                <span
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {(selectedBlock.params.rootNote ?? -1) >= 0 && (
                        <button
                          ...
                          title={
                            rootLocked ? 'Unlock root note' : 'Lock root note'
                          }
                        >
                    ...
                    <Dialog.Portal>
                      ...
                      >
                        ...
                        >
                          ...
                          />
                          <Dialog.Title
                            className="text-[13px] font-medium flex-1 m-0"
                            style={{ color: 'var(--color-text)' }}
                          >
                            Pitch Correction
                          </Dialog.Title>
                        ...
                        <div className="flex flex-col gap-4 px-5 py-4">
                          ...
                          <div className="flex items-center gap-6">
                            <div className="flex flex-col gap-1 flex-1">
                              <div className="flex items-center justify-between">
                                ...
                                >
                                  {Math.round(
                                    selectedBlock.params.correction ?? 80,
                                  )}{' '}
                                  %
                                </span>
                              </div>
                            ...
                            <div className="flex flex-col gap-1 flex-1">
                              <div className="flex items-center justify-between">
                                ...
                                >
                                  {Math.round(
                                    selectedBlock.params.humanize ?? 0,
                                  )}{' '}
                                  %
                                </span>
                              </div>
                          ...
                          >
                            ...
                            {BLACK_KEYS.map((note) => {
                              const isActive =
                                (pitchActiveNotes & (1 << note)) !== 0;
                              const whiteIdx =
                                WHITE_KEYS.findIndex((w) => w > note) - 1;
                              const offset =
                                (
                                  {
                                    1: 0.65,
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              >
                                {Math.round(
                                  (selectedBlock.params.shift ?? 0.5) * 48 - 24,
                                )}{' '}
                                st
                              </span>
                            </div>
                          ...
                          <div className="flex items-center justify-evenly gap-2">
                            <RotaryKnob
                              label="SHIFT"
                              value={selectedBlock.params.shift ?? 0.5}
                              min={0}
                              max={1}
                              size={48}
                              arcColor={pitchAccentColor}
                              formatValue={(v) =>
                                `${Math.round(v * 48 - 24)} st`
                              }
                              onChange={(v) =>
                                updateBlockParam(selectedBlock.id, 'shift', v)
                              }
                            />
                            <RotaryKnob
                              label="FINE"
                              value={selectedBlock.params.fine ?? 0.5}
                              min={0}
                              max={1}
                              size={48}
                              arcColor={pitchAccentColor}
                              formatValue={(v) =>
                                `${Math.round(v * 200 - 100)} ct`
                              }
                              onChange={(v) =>
                                updateBlockParam(selectedBlock.id, 'fine', v)
                              }
                            />
                            <RotaryKnob
                              label="FORMANT"
                              value={(selectedBlock.params.formant ?? 0) / 100}
                              min={0}
                              max={1}
                              size={48}
                              arcColor={pitchAccentColor}
                            ...
                            <RotaryKnob
                              label="F. FOLLOW"
                              value={
                                (selectedBlock.params.formantFollow ?? 0) / 100
                              }
                              min={0}
                              max={1}
                              size={48}
                            ...
                            <RotaryKnob
                              label="MIX"
                              value={(selectedBlock.params.mix ?? 100) / 100}
                              min={0}
                              max={1}
                              size={48}
                              arcColor={pitchAccentColor}
                ...
                <>
                    ...
                    >
                      {['Hz', 'Rate'].map((label, idx) => (
                        <button
                          key={label}
                          onClick={() =>
                            updateBlockParam(selectedBlock.id, 'rateMode', idx)
                          }
                          className="px-3 py-1 text-[10px] font-semibold cursor-pointer"
                          style={{
                            background:
                              (selectedBlock.params.rateMode ?? 0) === idx
                                ? selectedColor
                                : 'transparent',
                            color:
                              (selectedBlock.params.rateMode ?? 0) === idx
                  ...
                  {(selectedBlock.params.rateMode ?? 0) === 0 ? (
                    <RotaryKnob
                      ...
                      max={1}
                      onChange={(v) =>
                        updateBlockParam(selectedBlock.id, 'rate', v)
                      }
                      size={52}
                    ...
                    <RotaryKnob
                      ...
                      arcColor={selectedColor}
                      formatValue={(v) =>
                        RATE_LABELS[Math.round(v * 21)] ?? '1/3'
                      }
                    />
                  )}
                  ...
                  <RotaryKnob
                    ...
                    max={1}
                    onChange={(v) =>
                      updateBlockParam(selectedBlock.id, 'depth', v)
                    }
                    size={52}
                    arcColor={selectedColor}
                  ...
                  <RotaryKnob
                    ...
                    max={1}
                    onChange={(v) =>
                      updateBlockParam(selectedBlock.id, 'mix', v)
                    }
                    size={52}
                    arcColor={selectedColor}
                    formatValue={(v) => v.toFixed(2)}
                      ...
                      {['LP', 'BP', 'HP'].map((label, idx) => (
                        <button
                          ...
                          style={{
                            background:
                              Math.round(
                                selectedBlock.params.filterType ?? 1,
                              ) === idx
                                ? selectedColor
                                : 'transparent',
                            color:
                  ...
                  {selectedDef.params
                    ...
                    .map((p) => (
                      <RotaryKnob
                        ...
                        max={1}
                        onChange={(v) =>
                          updateBlockParam(selectedBlock.id, p.key, v)
                        }
                        size={52}
                        arcColor={selectedColor}
                        formatValue={(v) => v.toFixed(2)}
