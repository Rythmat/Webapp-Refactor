...
<nav className="flex gap-1 border-b border-white/10 pb-px">
  {tabs.map((t) => (
    <NavLink key={t.to} to={t.to} end={t.end}
      className={({ isActive }) => `px-3 py-2 text-sm font-medium transition-colors ${
...
- **Phase B — Insights/analytics (Overview/Grades):** completion rates, grade trends, engagement. A
  *light, display‑only* version (roster size, active students, assignment turned‑in counts from
