...
// Internal canvas resolution — animations are authored at 180×180
const INTERNAL_SIZE = 180;

export function AnimatedTrackIcon({
  animationId,
  size = 48,
}: {
  ...
  useEffect(() => {
    ...
    // Render at internal resolution, display at requested size via CSS
    canvas.width = INTERNAL_SIZE;
    canvas.height = INTERNAL_SIZE;

    const setup = animationLogic[animationId];
    if (!setup) return;

    let animationFrameId: number;
    const render = setup(ctx, INTERNAL_SIZE, INTERNAL_SIZE);
