 ...
 * Clicking any card opens the Main Globe and auto-starts that pathway.
 */
export const GlobePathways = () => {
      ...
      >
        ...
        <div className="flex flex-col gap-4 md:gap-5">
          {ROOTS_AND_ROUTES.map((route) => (
            <Link
              key={route.id}
              to={pathwayHref(route.id)}
              aria-label={`Trace ${route.title} on the globe`}
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {guidedTours.map((mod) => (
            <Link
              key={mod.id}
              to={pathwayHref(mod.id)}
              aria-label={`Start ${mod.title} on the globe`}
