...
function intervalsToString(intervals: number[]): string {
  const INTERVAL_NAMES: Record<number, string> = {
    ...
    12: '8', 13: 'b9', 14: '9', 15: '#9', 16: '10', 17: '11', 18: '#11',
  };
...
const FAMILY_MODES: Record<string, string[]> = {
  'Ionian': ['ionian', 'dorian', 'phrygian', 'lydian', 'mixolydian', 'aeolian', 'locrian'],
  'Harmonic Minor': ['harmonicMinor', 'locrianNat6', 'ionianSharp5', 'dorianSharp4', 'phrygianDominant', 'lydianSharp2', 'alteredDiminished'],
  'Melodic Minor': ['melodicMinor', 'dorianFlat2', 'lydianAugmented', 'lydianDominant', 'mixolydianFlat6', 'locrianNat2', 'altered'],
  'Harmonic Major': ['harmonicMajor', 'dorianFlat5', 'alteredDominantNat5', 'melodicMinorSharp4', 'mixolydianFlat2', 'lydianAugmentedSharp2', 'locrianDoubleFlat7'],
  'Double Harmonic Major': ['doubleHarmonicMajor', 'lydianSharp2Sharp6', 'ultraphrygian', 'doubleHarmonicMinor', 'oriental', 'ionianSharp2Sharp5', 'locrianDoubleFlat3DoubleFlat7'],
};
...
const MODE_DISPLAY: Record<string, string> = {
  ...
  harmonicMinor: 'Harmonic Minor', locrianNat6: 'Locrian \u266E6', ionianSharp5: 'Ionian #5',
  dorianSharp4: 'Dorian #4', phrygianDominant: 'Phrygian Dominant',
  lydianSharp2: 'Lydian #2', alteredDiminished: 'Altered Diminished',
  melodicMinor: 'Melodic Minor', dorianFlat2: 'Dorian \u266D2',
  lydianAugmented: 'Lydian Augmented', lydianDominant: 'Lydian Dominant',
  mixolydianFlat6: 'Mixolydian \u266D6', locrianNat2: 'Locrian \u266E2', altered: 'Altered',
  harmonicMajor: 'Harmonic Major', dorianFlat5: 'Dorian \u266D5',
  alteredDominantNat5: 'Altered Dominant \u266E5', melodicMinorSharp4: 'Melodic Minor #4',
  mixolydianFlat2: 'Mixolydian \u266D2', lydianAugmentedSharp2: 'Lydian Augmented #2',
  locrianDoubleFlat7: 'Locrian \u266D\u266D7',
  doubleHarmonicMajor: 'Double Harmonic Major', lydianSharp2Sharp6: 'Lydian #2 #6',
  ultraphrygian: 'Ultraphrygian', doubleHarmonicMinor: 'Double Harmonic Minor',
  oriental: 'Oriental', ionianSharp2Sharp5: 'Ionian #2 #5',
...
const MODE_TO_SLUG: Record<string, string> = {
  ionian: 'ionian', dorian: 'dorian', phrygian: 'phrygian', lydian: 'lydian',
  mixolydian: 'mixolydian', aeolian: 'aeolian', locrian: 'locrian',
  harmonicMinor: 'harmonic-minor', locrianNat6: 'locrian-nat6', ionianSharp5: 'ionian-sharp5',
  dorianSharp4: 'dorian-sharp4', phrygianDominant: 'phrygian-dominant',
  lydianSharp2: 'lydian-sharp2', alteredDiminished: 'altered-diminished',
  melodicMinor: 'melodic-minor', dorianFlat2: 'dorian-flat2',
  lydianAugmented: 'lydian-augmented', lydianDominant: 'lydian-dominant',
  mixolydianFlat6: 'mixolydian-flat6', locrianNat2: 'locrian-nat2', altered: 'altered',
  harmonicMajor: 'harmonic-major', dorianFlat5: 'dorian-flat5',
  alteredDominantNat5: 'altered-dominant-nat5', melodicMinorSharp4: 'melodic-minor-sharp4',
  mixolydianFlat2: 'mixolydian-flat2', lydianAugmentedSharp2: 'lydian-augmented-sharp2',
  locrianDoubleFlat7: 'locrian-double-flat7',
  doubleHarmonicMajor: 'double-harmonic-major', lydianSharp2Sharp6: 'lydian-sharp2-sharp6',
  ultraphrygian: 'ultraphrygian', doubleHarmonicMinor: 'double-harmonic-minor',
  oriental: 'oriental', ionianSharp2Sharp5: 'ionian-sharp2-sharp5',
...
function findAllInterpretations(degreeName: string): ChordInterpretation[] {
  ...
  const chordTones = qualityIntervals.map(qi => ((chordOffset + qi) % 12 + 12) % 12);

  const results: ChordInterpretation[] = [];

  for (const [familyName, familyIntervals] of Object.entries(FAMILY_INTERVALS)) {
    for (let m = 0; m < 7; m++) {
      const scalePcs = familyIntervals.map(i => (i - familyIntervals[m] + 12) % 12);
...
export function InsightContent() {
  ...
  const liveChord = useMemo(() => {
    ...
    const rootLetter = rootNote !== null ? noteNameInKey(rootPc, rootNote) : noteNameLetter(rootPc + 48);
    ...
    if (rootNote !== null) {
      ...
      if (degree > 0) {
        ...
        const chordParentRootPc = chordModeInfo ? (rootPc + chordModeInfo.offset) % 12 : rootPc;
        ...
        sessionMode = sessionDegIdx >= 0
          ? (FAMILY_MODES[chordParentFamily]?.[sessionDegIdx] ?? null) : null;
        ...
        alternatives = allInterps.filter(i => i.chordRootMode !== chordRootMode);
      }
    }
  ...
  const insights = useMemo(() => {
    ...
    for (const degreeName of stringSeq) {
      ...
      const noteNames = pitchedNotes.map((n) => noteNameInKey(n % 12, rootNote));
      ...
      const chordModeInfo = PARENT_SCALE_INFO[chordRootMode];
      const chordParentFamily = chordModeInfo?.family ?? 'Ionian';
      const chordParentRootPc = chordModeInfo ? (chordRootPc + chordModeInfo.offset) % 12 : chordRootPc;

      // Session key's mode from chord's perspective
      const sessionInterval = (rootNote - chordParentRootPc + 12) % 12;
      const chordFamilyIntervals = FAMILY_INTERVALS[chordParentFamily];
      const sessionDegIdx = chordFamilyIntervals?.indexOf(sessionInterval) ?? -1;
      const sessionMode = sessionDegIdx >= 0
        ? (FAMILY_MODES[chordParentFamily]?.[sessionDegIdx] ?? null) : null;
      ...
      // All family interpretations (excluding primary)
      const allInterps = findAllInterpretations(degreeName);
      const alternatives = allInterps.filter(i => i.chordRootMode !== chordRootMode);
  ...
  const [kr, kg, kb] = hasProgression ? KEY_COLORS[keyIdx] : [255, 255, 255] as const;
    ...
    <div className="flex flex-col gap-0.5">
      ...
      >
        <div className="flex items-center gap-1.5">
          {liveChord?.color && (
            <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: liveChord.color }} />
          )}
        ...
        {liveChord ? (
          <>
              ...
              <div className="flex gap-1 flex-wrap">
                {liveChord.intervals.split(' ').map((interval, i) => (
                  <span key={i} className="text-[9px] font-mono px-1 py-0.5 rounded"
                    style={{ backgroundColor: 'var(--color-surface-2)', color: interval === 'R' ? 'var(--color-accent)' : 'var(--color-text-dim)' }}>
                    {interval}
                  </span>
                ))}
              </div>
            ...
            {liveChord.chordRootMode && (
              <div className="text-[9px] leading-snug" style={{ color: 'var(--color-text-dim)' }}>
                {liveChord.rootLetter} {MODE_DISPLAY[liveChord.chordRootMode] ?? liveChord.chordRootMode}
                {liveChord.sessionMode && keyLetter &&
                  !(liveChord.sessionMode === liveChord.chordRootMode && liveChord.rootLetter === keyLetter) && (
                  <> · {keyLetter} {MODE_DISPLAY[liveChord.sessionMode] ?? liveChord.sessionMode}</>
                )}
                {!liveChord.isSessionParent && liveChord.parentKeyLetter && liveChord.parentMode && (
                  <> · Parent: {liveChord.parentKeyLetter} {MODE_DISPLAY[liveChord.parentMode] ?? liveChord.parentMode}</>
                )}
              </div>
            )}
            {liveChord.alternatives.length > 0 && (
              <div className="flex flex-col gap-0.5 mt-0.5">
                <button
                  ...
                  style={{ color: 'var(--color-text-dim)', background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}
                >
                  <ChevronDown
                    size={10}
                    strokeWidth={2}
                    style={{ transition: 'transform 150ms', transform: showLiveAlts ? 'rotate(180deg)' : 'rotate(0deg)' }}
                  />
                  Also found in {liveChord.alternatives.length} scale{liveChord.alternatives.length > 1 ? 's' : ''}
                </button>
                {showLiveAlts && liveChord.alternatives.map((alt, i) => (
                  <div key={i} className="flex items-center gap-1.5 pl-3 text-[9px] leading-snug"
                    style={{ color: 'var(--color-text-dim)', opacity: 0.7 }}>
                    <span>{liveChord.rootLetter} {MODE_DISPLAY[alt.chordRootMode] ?? alt.chordRootMode} ({alt.family})</span>
                    <Link
                      to={LearnRoutes.lesson({ mode: MODE_TO_SLUG[alt.chordRootMode] ?? alt.chordRootMode, key: keyLabelToUrlParam(liveChord.rootLetter) })}
                      ...
                      onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(126,207,207,0.15)'; }}
            ...
            {liveChord.chordRootMode && liveChord.rootLetter && (
              <Link
                to={LearnRoutes.lesson({ mode: MODE_TO_SLUG[liveChord.chordRootMode] ?? liveChord.chordRootMode, key: keyLabelToUrlParam(liveChord.rootLetter) })}
                className="flex items-center gap-1 self-start mt-0.5 px-1.5 py-0.5 rounded text-[9px] font-medium transition-colors"
                style={{ backgroundColor: 'var(--color-surface-3)', color: 'var(--color-accent)' }}
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(126,207,207,0.15)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'var(--color-surface-3)'; }}
              >
                <BookOpen size={9} strokeWidth={2} />
                Learn {liveChord.rootLetter} {MODE_DISPLAY[liveChord.chordRootMode] ?? liveChord.chordRootMode}
              </Link>
            )}
          </>
        ) : (
      ...
      {hasProgression ? (
        <>
          ...
          {insights.map((chord) => (
            ...
            >
                  ...
                  {chord.intervals.split(' ').map((interval, i) => (
                    <span
                      ...
                      style={{
                        ...
                        color: interval === 'R' ? 'var(--color-accent)' : 'var(--color-text-dim)',
                      }}
                    >
                      {interval}
                    </span>
              ...
              {/* Mode info */}
              <div className="text-[9px] leading-snug" style={{ color: 'var(--color-text-dim)' }}>
                {chord.rootLetter} {MODE_DISPLAY[chord.chordRootMode] ?? chord.chordRootMode}
                {chord.sessionMode && keyLetter &&
                  !(chord.sessionMode === chord.chordRootMode && chord.rootLetter === keyLetter) && (
                  <>{' · '}{keyLetter} {MODE_DISPLAY[chord.sessionMode] ?? chord.sessionMode}</>
                )}
                {!chord.isSessionParent && (
                  <> · Parent: {chord.parentKeyLetter} {MODE_DISPLAY[chord.parentMode] ?? chord.parentMode}</>
                )}
              </div>
                ...
                <div className="flex flex-col gap-0.5 mt-0.5">
                  ...
                  >
                    <ChevronDown
                      size={10}
                      strokeWidth={2}
                      style={{ transition: 'transform 150ms', transform: expandedCards.has(chord.degreeName) ? 'rotate(180deg)' : 'rotate(0deg)' }}
                    />
                    Also found in {chord.alternatives.length} scale{chord.alternatives.length > 1 ? 's' : ''}
                  </button>
                  {expandedCards.has(chord.degreeName) && chord.alternatives.map((alt, i) => (
                    <div key={i} className="flex items-center gap-1.5 pl-3 text-[9px] leading-snug"
                      style={{ color: 'var(--color-text-dim)', opacity: 0.7 }}>
                      <span>{chord.rootLetter} {MODE_DISPLAY[alt.chordRootMode] ?? alt.chordRootMode} ({alt.family})</span>
                      <Link
                        to={LearnRoutes.lesson({ mode: MODE_TO_SLUG[alt.chordRootMode] ?? alt.chordRootMode, key: keyLabelToUrlParam(chord.rootLetter) })}
                        className="flex items-center gap-0.5 px-1 py-0.5 rounded text-[8px] font-medium transition-colors shrink-0"
                        style={{ backgroundColor: 'var(--color-surface-2)', color: 'var(--color-accent)' }}
                        onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(126,207,207,0.15)'; }}
                        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'var(--color-surface-2)'; }}
              ...
              {/* Learn link */}
              <Link
                to={LearnRoutes.lesson({ mode: MODE_TO_SLUG[chord.chordRootMode] ?? chord.chordRootMode, key: keyLabelToUrlParam(chord.rootLetter) })}
                className="flex items-center gap-1 self-start mt-0.5 px-1.5 py-0.5 rounded text-[9px] font-medium transition-colors"
                style={{
              ...
              >
                <BookOpen size={9} strokeWidth={2} />
                Learn {chord.rootLetter} {MODE_DISPLAY[chord.chordRootMode] ?? chord.chordRootMode}
              </Link>
            </div>
          ))}
        </>
        ...
        <div className="flex flex-col items-center justify-center gap-3 px-4 py-10 text-center">
          <Lightbulb size={20} strokeWidth={1.5} style={{ color: 'var(--color-text-dim)' }} />
          <span className="text-[10px] leading-relaxed" style={{ color: 'var(--color-text-dim)' }}>
