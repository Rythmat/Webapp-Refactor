...
function filterEvents(parsed: ParsedQuery, query: string): HistoricalEvent[] {
  const hasCriteria =
    parsed.decade != null || parsed.city != null || parsed.genres.length > 0;
  ...
  let results = MUSIC_HISTORY.filter((event) => {
    ...
    if (parsed.city != null) {
      if (event.location.city.toLowerCase() !== parsed.city.name.toLowerCase())
        return false;
    }
    if (parsed.genres.length > 0) {
      const eventGenresLower = event.genre.map((g) => g.toLowerCase());
      if (
        !parsed.genres.some((g) => eventGenresLower.includes(g.toLowerCase()))
      )
        return false;
    }
    return true;
  ...
  if (results.length === 0) {
    const scored = MUSIC_HISTORY.map((event) => {
      ...
      if (
        parsed.city != null &&
        event.location.city.toLowerCase() === parsed.city.name.toLowerCase()
      )
        score += 3;
      if (parsed.genres.length > 0) {
        const eventGenresLower = event.genre.map((g) => g.toLowerCase());
