  ...
  return (
    ...
    >
      <div
        ...
        style={{ marginBottom: 'clamp(0.2rem, 0.4vw, 0.4rem)' }}
      >
        <span
      ...
      >
        <div
          className="rounded-lg overflow-hidden flex-shrink-0 self-stretch"
          style={{ aspectRatio: '1', width: 'auto' }}
        >
          <HexAvatarSVG
            config={defaultAvatarConfig(artSeed)}
            circular={false}
        ...
        <div className="flex-1 flex flex-col justify-between min-w-0 py-0.5">
          ...
          <div className="flex flex-col justify-evenly flex-1 mt-1">
            {[
              ...
              {
                name: 'XP',
                value: Math.min(100, (xp / maxXp) * 100),
                color: '#FFCC33',
                suffix: `${xp} XP`,
              },
            ].map((bar) => (
              <div
                ...
                style={{ gap: 'clamp(0.15rem, 0.3vw, 0.25rem)' }}
              >
                <span
                  ...
                  style={{
                    ...
                    color: bar.color,
                    minWidth: 'clamp(2.8rem, 4.5vw, 4rem)',
                  }}
                >
                  {bar.name}
                </span>
                ...
                <span
                  className="flex-shrink-0 tabular-nums text-white/60"
                  style={{
                    fontSize: 'clamp(0.4rem, 0.6vw, 0.5rem)',
                    minWidth: 'clamp(1.5rem, 2.5vw, 2.5rem)',
                    textAlign: 'right',
                  }}
                >
...
export const LearnDashboard: FC<LearnDashboardProps> = ({ navigate }) => {
  ...
  const styleData = useMemo(() => {
    const lessons =
      progressSummary?.lessons?.filter(
        (l) =>
          l.lessonId.startsWith('curriculum:') &&
          !l.lessonId.includes('piano-fundamentals'),
      ) ?? [];
    if (lessons.length === 0)
      return {
        label: 'No courses started',
        artSeed: 'rock',
        completion: 0,
        xp: 0,
      };
    ...
    const genreLessons = lessons.filter((l) =>
      l.lessonId.startsWith(`curriculum:${genreId}:`),
    );
  ...
  const theoryData = useMemo(() => {
    ...
    return {
      label: modeName,
      artSeed: (latest.mode ?? 'ionian').toLowerCase(),
      completion,
      xp: latest.completedCount * 5,
    };
  }, [progressSummary]);

  /* ── Compute Technique card data ── */
  const techniqueData = useMemo(() => {
    const lesson = progressSummary?.lessons?.find((l) =>
      l.lessonId.includes('piano-fundamentals'),
    );
    if (!lesson) return { completion: 0, xp: 0 };
    const completion =
      lesson.totalCount && lesson.totalCount > 0
  ...
  return (
    ...
    >
          ...
          {TABS.map((tab) => (
            <button
              ...
              style={{
                ...
                background:
                  activeTab === tab ? '#ffffff' : 'rgba(255,255,255,0.06)',
      ...
      {/* ── Main Content ── */}
      <div
        className="flex-1 min-h-0 lg:overflow-hidden overflow-y-auto custom-scrollbar flex flex-col"
        style={{
          padding: '0 clamp(0.5rem, 1.2vw, 1rem) clamp(0.5rem, 1vw, 0.75rem)',
        }}
      >
        {/* ── Top Row: 3 Progress Cards ── */}
        <div
          className="grid grid-cols-1 sm:grid-cols-3 flex-shrink-0"
          style={{
            gap: 'clamp(0.4rem, 0.7vw, 0.6rem)',
            marginBottom: 'clamp(0.4rem, 0.7vw, 0.6rem)',
          }}
        >
          <ProgressCard
            title="Style"
            artSeed={styleData.artSeed}
            label={styleData.label}
            completion={styleData.completion}
            accuracy={90}
            xp={styleData.xp}
            maxXp={Math.max(styleData.xp, 500)}
            onClick={() => navigate('/learn?tab=Courses')}
          />
          <ProgressCard
            title="Theory"
            artSeed={theoryData.artSeed}
            label={theoryData.label}
            completion={theoryData.completion}
            accuracy={90}
            xp={theoryData.xp}
            maxXp={Math.max(theoryData.xp, 500)}
            onClick={() => navigate('/learn?tab=Theory')}
          />
          <ProgressCard
            title="Technique"
            artSeed="technique"
            label="Fundamentals"
            completion={techniqueData.completion}
            accuracy={90}
            xp={techniqueData.xp}
            maxXp={Math.max(techniqueData.xp, 300)}
            onClick={() => navigate('/learn?tab=Technique')}
          />
        </div>

        {/* ── Bottom Row: Songs + Challenges ── */}
        <div
          className="grid grid-cols-1 md:grid-cols-[3fr_2fr] flex-1 min-h-0"
          style={{ gap: 'clamp(0.4rem, 0.7vw, 0.6rem)' }}
        >
          {/* Songs Table */}
          <div
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  padding:
                    'clamp(0.2rem, 0.35vw, 0.3rem) clamp(0.3rem, 0.6vw, 0.5rem)',
                }}
              >
                <Search
                  className="text-white/30 flex-shrink-0"
                  style={{
                    width: 'clamp(0.6rem, 0.9vw, 0.75rem)',
                    height: 'clamp(0.6rem, 0.9vw, 0.75rem)',
                  }}
                />
                <input
                  type="text"
            ...
            {/* Table header */}
            <div
              className="flex items-center text-white/30"
              style={{
                fontSize: 'clamp(0.4rem, 0.6vw, 0.5rem)',
                marginBottom: 'clamp(0.2rem, 0.3vw, 0.25rem)',
            ...
            <div className="flex flex-col flex-1 justify-evenly overflow-hidden">
              {filteredSongs.map((song) => (
                <div
                  ...
                  style={{
                    padding:
                      'clamp(0.2rem, 0.4vw, 0.35rem) clamp(0.2rem, 0.3vw, 0.25rem)',
                    gap: 'clamp(0.3rem, 0.5vw, 0.4rem)',
                  }}
                  onClick={() => {
                    play('click');
          ...
          >
            ...
            >
              <Ico
                ...
                style={{
                  width: 'clamp(0.6rem, 0.85vw, 0.75rem)',
                  height: 'clamp(0.6rem, 0.85vw, 0.75rem)',
                }}
              />
              <span
                className="text-white font-semibold"
            ...
            <div className="flex flex-col flex-1 justify-evenly">
              {CHALLENGES.map((c) => (
                <div key={c.name} className="flex items-center justify-between">
                  <div
                    className="flex items-center"
                    style={{ gap: 'clamp(0.25rem, 0.4vw, 0.35rem)' }}
                  >
                    <div
